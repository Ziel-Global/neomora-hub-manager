...
//           {["Inquiry", "Documents Pending", "Fee Pending", "Active", "On Hold", "Completed", "WaitList", "Withdrawn"].map((s) => (
...
//           <SelectItem value="summer">4:30 to 5:30 PM</SelectItem>
//           <SelectItem value="winter">5:45 to 7:00 PM</SelectItem>
//         </SelectContent>
//       </Select>
...
import { Download, Eye, Pencil, Check, X, ChevronLeft, ChevronRight } from "lucide-react";
import { toast } from "sonner";
import { PageHeader } from "@/components/PageHeader";
import { DataTable, type Column } from "@/components/DataTable";
import { StatusBadge } from "@/components/StatusBadge";
...
// Locations are derived from the real registers (Riyadh / Jeddah) rather than the old mock list.
...
function ParticipantsPage() {
  ...
  const filtered = useMemo(() => {
    const q = search.toLowerCase().trim();
    const rows = mockParticipants.filter((p) => {
      if (locFilter !== "all" && p.location !== locFilter) return false;
      if (dayFilter !== "all" && p.day.toUpperCase() !== dayFilter) return false;
      if (sesFilter !== "all" && p.ageGroup !== sesFilter) return false;
      if (statusFilter !== "all" && p.status !== statusFilter) return false;
      if (q) {
        const hay = `${p.fullName} ${p.guardianPhone} ${p.uniqueId}`.toLowerCase();
        if (!hay.includes(q)) return false;
    ...
    // Prioritize: group by Location, then within each location surface
    // Active participants before WaitList entries.
  ...
  }, [locFilter, dayFilter, sesFilter, statusFilter, search]);

  useEffect(() => { setPage(1); }, [locFilter, dayFilter, sesFilter, statusFilter, search]);
  ...
  const ageGroups = useMemo(
    () => Array.from(new Set(mockParticipants.map((p) => p.ageGroup))).filter(Boolean).sort(),
    []
  );

  const waitlistCount = useMemo(
    () => mockParticipants.filter((p) => p.status === "WaitList").length,
    []
  );
  ...
  const columns: Column<MockParticipant>[] = [
    ...
    { key: "guardianPhone", header: "CONTACT NUMBER" },
    {
      ...
      render: (r) => (
        <div className="flex gap-1">
          {(r.status === "Inquiry" || r.status === "WaitList" || r.registrationData) && (
            <>
              {/* <Button
                variant="ghost"
                size="icon"
  ...
  const filters = (
    <>
      ...
      <Select value={sesFilter} onValueChange={setSesFilter}>
        <SelectTrigger className="w-[190px]"><SelectValue placeholder="Age Group / Session" /></SelectTrigger>
        <SelectContent>
  ...
  return (
    <>
      <PageHeader
        title="Participants"
        description={`${mockParticipants.length} total \u2022 ${waitlistCount} on WaitList`}
        actions={
          <>
            <Button variant="outline"><Download className="mr-2 h-4 w-4" /> Export</Button>
...
function mapStatus(s: string): string {
  // StatusBadge styles: Active/Pending/On Hold/Withdrawn/Completed
  if (s === "Documents Pending" || s === "Fee Pending" || s === "Inquiry" || s === "WaitList") return "WaitList";
  return s;
}
        ...
        <Tabs defaultValue="overview">
          ...
          <TabsContent value="overview" className="mt-4">
            <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              <Info label="Age Group / Session" value={p.ageGroup} />
              <Info label="With Kit" value={p.withKit ? "Yes" : "No"} />
              <Info label="Guardian" value={p.guardianName} />
              <Info label="Phone" value={p.guardianPhone} />
              <Info label="Email" value={p.guardianEmail} />
            </div>
            ...
            {/* {(p.status === "Inquiry" || p.status === "WaitList" || p.registrationData) && (
              <div className="mt-8 flex gap-3">
                <Button
                  className="flex-1 bg-green-600 hover:bg-green-700 text-white"
                  onClick={() => toast.success("Participant has been accepted!")}
